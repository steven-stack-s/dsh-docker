...
- **Program decoupled from image, upgraded in-container** — the DSH program lives on a mounted volume; daily upgrades are `docker exec dsh npm install -g @deepseek-ai/dsh@<version> && docker restart dsh` — no image rebuild needed. Note that the **image layer and dsh itself are two separate layers**: `docker compose pull && up -d` updates only the image (entrypoint / rescue scripts) and does **not** change the dsh in the volume (see [docs 03](docs/en/03-upgrade-maintenance.md)).
...
Release history in [CHANGELOG.md](CHANGELOG.md). Image tags follow the dual-version scheme `v<project-version>-dsh-<dsh-version>` (e.g. `v0.4.9-dsh-0.1.6-alpha.2`); pushing a tag in that format auto-builds multi-arch images to `ghcr.io`.
